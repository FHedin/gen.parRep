...
simulation =
{
  ...
  algorithm = "PARREP_FV",
  -- It is possible to allow more than one exit event to be generated by the parallel phase of the algorithm
  --  in that case allowedExitEvents should be set to something large than 1 ; by default it is internally set to 1
  allowedExitEvents = 25,
  -- In case you allow more than one exit event above, you need to decide if the exiting ranks will wait for the others at the next mpi barrier, of if
  --  they should continue to run: in that case they will cone the config of one of the non exiting ranks and continue
  -- Set the following variable either to "wait" or "clone" ; default is "wait"
  afterExit = "clone" ,
...
-- It may be necessary to communicate to c++ information concerning the states status (the multiExit parRep with cloning for example requires this for doing a serialization)
-- In this case the following function is called from c++ side.
--
-- this function is mandatory and called from C++, program will fail if not defined
  --  it should take no arguments
  --  it should return a table where you save any data you want 
function get_serialized_state()
  
  local status = {}
  
  -- add things to table status if required here
  -- ...
  
  status.from = fromState
  status.to   = toState
...
-- this function is mandatory and called from C++, program will fail if not defined
  --  it should take one argument, the table defined in get_serialized_state()
  --  it should return nothing
function put_serialized_state(status)
  -- do here unserialization of what was serialized above in get_serialized_state()
  -- ...
  
  fromState = status.from
  toState   = status.to
...
then
  ...
  --         -- ...
        elseif key=="ignore_ref_time" then
          -- in some cases the exit time might be inserted before the reference_time is known: in that case the variable is set to NaN
