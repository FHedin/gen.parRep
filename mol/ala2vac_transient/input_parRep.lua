...
-- ParRep (original A. Voter algorithm): only "algorithm" is mandatory, the others have default (c.f. documentation)
simulation =
{
  -- The parallel replica algorithm as defined by A. Voter et al.
  algorithm = "PARREP",
  -- parameters for the ParRep decorrelation stage
  --  tauDecorr is an estimate of the time required for decorrelation (unit : picoseconds)
  --  checkDecorr is the frequency (in steps) at which to verify if the system left the current state
  tauDecorr = 100.0, -- 100.0 ps
  checkDecorr = 500, -- 1.0 ps
  -- parameters for the ParRep dephasing stage
  --  tauDephase is an estimate of the time required for dephasing (unit : picoseconds)
  --  checkDephase is the frequency (in steps) at which to verify if the system left the current state
  tauDephase = 100.0, -- 100.0 ps
  checkDephase = 500, -- 1.0 ps
...
function state_init()
  ...
  -- we define 2 states 'C_ax' and 'C_eq', and the UNK which will trigger transient propagation
  ...
  local domain = nil
  if( (phi > 0.0 and phi < 120.0) and (psi < 0.0 and psi > -170.0) ) then
    domain = 'C_ax'
  elseif( (phi < -20.0) or (phi > 140.0 and (psi < -20.0 or psi > 80.0)) ) then
    domain = 'C_eq'
  else
    domain = 'UNK'
  end
  ...
  print("Initial state is: "..fromState.." {"..phi.." "..psi.."} ")
...
function check_transient_propagation_required()
  ...
  print("(phi,psi) =",phi,",",psi," --> need transient ? ",need)
...
database = 
{
  ...
  backupFrequency = 1e99
}

-- functions for storing data to a database are defined as members of the table SQLiteDB
