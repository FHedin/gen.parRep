...
function state_init()
  ...
  if( (phi > 0.0 and phi < 120.0) and (psi < 0.0 and psi > -170.0) ) then
    domain = 'C_ax'
  -- then check if within a rectangular domain wrapping the C_eq valley
...
-- For debugging purpose: force a fake exit of the ref walker with probability 1%
--   if(mpi_rank_id==0) then
--     local prob = math.random()
--     if (prob <= 0.01) then escaped = true end
--   end
